...
from transformers import AdamW, AutoConfig, AutoModel, RobertaTokenizer
...
class LitModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        output_state = roberta_output[0]
        # weights = self.attention(output_state)
        # output_state = weights * output_state
        input_mask_expanded = (
            attention_mask.unsqueeze(-1).expand(output_state.size()).float()
        )
        sum_embeddings = torch.sum(output_state * input_mask_expanded, 1)
...
if __name__ == "__main__":
    ...
    for index in range(5):
        model_path = f"../input/commonlit-min-max-weighted/model_{index + 1}.pth"
        print(f"\nUsing {model_path}")
    ...
    np.save("shigeria_pred4", predictions.reshape(-1, 1))
