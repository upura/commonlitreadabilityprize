...
class LitModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        input_mask_expanded = (
            attention_mask.unsqueeze(-1).expand(cat_over_last_layers.size()).float()
        )
        sum_embeddings = torch.sum(cat_over_last_layers * input_mask_expanded, 1)
        ...
        return self.linear(
            torch.cat([cls_pooling, norm_mean_embeddings, context_vectors], -1)
        )
...
if __name__ == "__main__":
    ...
    for index in range(5):
        model_path = f"../input/commonlit-concat-mean-attention-pooling-three/model_{index + 1}.pth"
        print(f"\nUsing {model_path}")
    ...
    np.save("shigeria_pred10", predictions.reshape(-1, 1))
