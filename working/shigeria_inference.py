...
if __name__ == "__main__":
    ...
    svd2 = TruncatedSVD(n_components=3, n_iter=10, random_state=42)
    svd1.fit(X_train)
    svd2.fit(X_test)
    X_train_svd = svd1.transform(X_train)
    X_test_svd = svd2.transform(X_test)

    X_train_ = np.concatenate(
        [
            X_train_svd,
            andrey_df.reshape(-1, 1),
            andrey_df2.reshape(-1, 1),
            andrey_df3.reshape(-1, 1),
        ],
        axis=1,
    )
    X_test_ = np.concatenate(
        [
            X_test_svd,
            model1_predictions.reshape(-1, 1),
            model2_predictions.reshape(-1, 1),
            model3_predictions.reshape(-1, 1),
        ],
        axis=1,
    )
    clf = ARDRegression(
    ...
    clf.fit(X_train_, train_df.target)
