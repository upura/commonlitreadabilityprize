...
class LitModel(nn.Module):
    def __init__(self):
        ...
        self.attention = nn.Sequential(
            nn.Linear(4096, 512), nn.Tanh(), nn.Linear(512, 1), nn.Softmax(dim=1)
        )
    ...
    def forward(self, input_ids, attention_mask):
        ...
        cat_over_last_layers = torch.cat(
            (output_state[-1], output_state[-2], output_state[-3], output_state[-4]), -1
        )
...
if __name__ == "__main__":
    ...
    for index in range(5):
        model_path = (
            f"../input/commonlit-attention-pooling-dataset/model_{index + 1}.pth"
        )
        print(f"\nUsing {model_path}")
    ...
    np.save("shigeria_pred8", predictions.reshape(-1, 1))
